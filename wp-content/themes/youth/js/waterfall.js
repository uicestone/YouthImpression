            ...
            if(!(isFinish || isLoading)){ // 确保上一次加载完毕才发送新的请求
                ...
                if(colsHeight.minHeight + wf_box_top < $(window).height() + $(window).scrollTop()){
                    ...
                    if(jsonCache.length > 0){
                        ...
                        if(opts.ajaxTimes === 'infinite' || ajaxTimes < opts.ajaxTimes || !needManual){
                            showMsg('loading');
                            // 传参给服务器
        ...
        function showMsg(type){
            switch(type){
                ...
                case 'error':
                    ...
                    break;
                case 'manual':
                    var showmore = $('<span style="cursor:pointer" class="showmore">点击加载更多</span>');
                    $wf_result.removeClass('wf_loading').show().empty().append(showmore);
